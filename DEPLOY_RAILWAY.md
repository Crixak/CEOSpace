...
4. En **Settings → Networking**, click en `Generate Domain` para tener una URL pública (algo como `https://palacio-jamon-server.up.railway.app`). Copiala, la vas a necesitar en el paso 4.
...
3. En **Variables**, agregá:
   - `VITE_API_URL` → la URL pública del backend que copiaste en el paso 3 (ej. `https://palacio-jamon-server.up.railway.app`). Este valor se usa en build-time (queda "horneado" en el bundle), así que si cambia la URL del backend hay que volver a hacer deploy del frontend.
...
5. Deploy. Esa URL pública (`https://palacio-jamon-client.up.railway.app`) es la que le compartís a tu compañero para que vaya viendo el avance.

## 5. Probar

Entrá a la URL del frontend y logueate con alguno de los usuarios de prueba del seed:

- `admin@palaciodeljamon.com` / `palacio123`
- `encargado.centro@palaciodeljamon.com` / `palacio123`
- `vendedor.centro@palaciodeljamon.com` / `palacio123`
